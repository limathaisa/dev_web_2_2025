document.addEventListener("DOMContentLoaded", (ev) => {
  ...
  formCad.addEventListener("submit", (ev2) => {
    ...
    validaFormulario(campoNome.value, campoPreco.value)
      ? formCad.submit()
      : null;
  });
...
let validaPreco = (campoPreco, charDigitado) => {
  if (
    ...
    campoPreco.value = campoPreco.value.substring(
      0,
      campoPreco.value.length - 2
    );
  }
  //SE A VÍRGULA JÁ FOI DIGITADA ANTES, TAMBÉM TIRA
  if (
    charDigitado == "," &&
    campoPreco.value.indexOf(",") < campoPreco.value.length - 1
  ) {
    campoPreco.value = campoPreco.value.substring(
      0,
      campoPreco.value.length - 1
    );
  }
};
